$(document).ready(function (){
    ...
    $('#FormReg').submit(function (e){
        e.preventDefault();
        let form = $(this)
        let first_name = form.find("input[name='first_name']").val()          //    find - поиск
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == 'ok'){
                        $('#result').text("Решистрация прошла успешно!")
                    } else if (response.status == 'NoLogin'){
                        $('#result').text("Решистрация не прошла! Неверный логин")
                    } else {
                        $('#result').text("Упсс... что-то пошлоне так!")
                    }
                }
            })
        } else {
